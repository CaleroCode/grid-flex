...
describe("Colecciones en JS: Array", () => {
    ...
    it("Pueden crearse usando new Array", () => {
        ...
        let a = new Array("primero", "segundo");
        expect(Array.isArray(a)).to.be.true;
    })
    ...
    it("Nos permiten modificar cada elemento", () => {
        ...
        fruits[1] = "Pera"; // Cambiamos "Manzana" por "Pera"
    ...
    it("Podemos añadir elementos", () => {
        ...
        fruits.push("Sandía");
        expect(fruits).to.have.same.members(["Piña", "Manzana", "Fresa", "Melón", "Sandía"])
    })
})
...
it("Filtrando colores", () => {

    let colors = ["red", "blue", "green", "yellow", "pink", "black", "purple", "orange"];
    let result = colors
                    .filter(color => color.includes('e'))
                    .map(color => `Color: ${color.charAt(0).toUpperCase() + color.slice(1)}`)
